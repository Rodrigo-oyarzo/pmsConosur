...
//version 9.35: persistencia promociones
//version 9.34: bug fecha de incio
//version 9.33: ms big fecha negcio
//version 9.31: bug diadenogcio
//version 9.3: envia dia de negocio en redencion cupon
//version 9.2: mostrar efectivo en cajon asignado, llamada a hasar en clarin365
...
Event Idle_No_trans
    ...
    ELSEIF (aux<gUltMinComunicacion)
        ...
        call darModeloCaja
        gUltMinComunicacion=aux  
    ELSEIF (gUltMinComunicacion<0 or gUltMinComunicacion>1440)
        gUltMinComunicacion=0
        Call setFilePaths()
        call darModeloCaja
        call sqlDiaDeNegocio
    ENDIF
...
Event inq: 19
    ...
    call aplicaDescuentoLocal(224,3,7250,50)
endevent

Event inq: 20
    //descobj,limitediario,limite en dinero, descuento
    call aplicaDescuentoLocal(204,2,9200,100)
endevent
...
Event inq: 30 //poner en hora la fiscal, se llama desde una macro
    ...
    call SyncDateTime
Endevent
//********************* Monto efectivo en caja asignada ***************************
Event inq: 31
    IF (@cashdrawer=0 and @Emplopt[4]=0)
        ExitWithError ("ERROR: NO TIENE ASIGNADA LA CAJA")
    ELSE
	...
	If dbok
		...
		If (error="")
                    DLLCall_CDECL gDBhdl, sqlGetFirst(ref resultado) 
                     Split resultado, ";",monto
		Else 	
                    
                    ErrorMessage "PMS5: ValidarEfectivo: Error al consulta efectivo"
                    ErrorMessage error
		Endif	
                call ODBCcerrarconexion()
                call ODBCbaja
	Else
            ErrorMessage "PMS5: ValidarEfectivo: Error al conectar BD"
	Endif		
    ENDIF
endevent
//*********************** DIAS *******************************
...
Sub inicioPos
    ...
    format gDatos as "INITPOS|",@WSID,"|",gVersion,"|",gCaja,"|",@RVC
...
Sub recibeComunicacion
    ...
    format gDatos as "COMUNICACION|",gIdComunicacion,"|",@WSID,"|",gVersion,"|",gCaja,"|",gUltMinComunicacion,"|",gFechaHoy,"|",gErrorDB
...
sub enviarCodigosOffline
    ...
    If (gOffCantCodIngresados>0)
        ...
        WHILE ((i>0) and (gOffRecibido=1))
            format gDatos as "OFFCOD|",@WSID,"|",gOffCodIngresados[i],"|",@RVC,"|",gFechaHoy
            call EnviaTransaccion
	...
	if @RxMsg = "_timeout" //Llega la Respuesta
	   //InfoMessage "Validacion","NO HAY COMUNICACION CON SERVICIO DE VALIDACION"
	    ...
	    Split @RxMsg, "|", gContadorOff,gOffCamp[]:gOffTipo[]:gOffLen[]:gOffCodMicros[]:gOffRvc[]:gOffCodMicrosNivel[]
            ...
            call grabarCampanasOffline
	endif
	
endsub
//************************* OFFLINE ******************************
// Graba las campanas offline
//****************************************************************
SUB grabarCampanasOffline
    VAR i: N4
    Var handle	: N5  
    VAR ArchConfig:A100
    ...
    FORMAT ArchConfig AS g_path, "CAMPANAS", ".txt"
        
    FOPEN handle, ArchConfig, write

    IF handle <> 0
        FOR i=1 TO gContadorOff
            FWRITE handle, gOffCamp[i],gOffTipo[i],gOffLen[i],gOffCodMicros[i],gOffRvc[i],gOffCodMicrosNivel[i]
        ENDFOR
        FCLOSE handle
    ELSE
            ErrorMessage "No pude grabar campanas offline"
    ENDIF

ENDSUB
//************************* OFFLINE ******************************
// lee las campanas offline
//****************************************************************
SUB leerCampanasOffline
    gContadorOff=0
    Var handle	: N5  
    VAR ArchConfig:A100
    ...
    IF handle <> 0
        WHILE NOT FEOF( Handle )
            gContadorOff=gContadorOff+1
           FREAD Handle, gOffCamp[gContadorOff],gOffTipo[gContadorOff],gOffLen[gContadorOff],gOffCodMicros[gContadorOff],gOffRvc[gContadorOff],gOffCodMicrosNivel[gContadorOff]
           IF (  FEOF( Handle ) )
                gContadorOff=gContadorOff-1
           ENDIF
       ENDWHILE
...
sub procesaCupones(Var porTarjeta: N1,Var prefijo: A5, VAR banda: N1)
    ...
    ELSE
        format aux as @DAY,""
        IF (mid(aux,2,1)<>mid(gFechaHoy,10,1))
            call sqlDiaDeNegocio
        ENDIF
    ENDIF
   // IF (porTarjeta=0)
   //     call consultarCodigo(gCodigoIngresado)
    ...
    if (gCodigoIngresado="") 
     exitwitherror"DEBE INGRESAR UN CODIGO"
        ...
        format gDatos as "CUPON|",@WSID,"|",gCodigoIngresado,"|",gVersion,"|",gCaja,"|",porTarjeta,"|",@RVC,"|",gFechaHoy,"|",gErrorDB
    //@WSType
...
Sub procesarMenuOpciones
	...
	If kKeyPressed = @KEY_ENTER
                IF opcion>0 and opcion<=gContadorProd 
                     format gDatos as "CUPONOPC|",@WSID,"|",gCodigoIngresado,"|",gVersion,"|",gCaja,"|",gCodigos[opcion],"|",gCodigosNivel[opcion],"|",gfechaHoy
                     call EnviaTransaccion
                     call RecibeTransaccion
                     call procesarRespuesta
                     IF (gCodigoRespuesta="3") 
	...
	If dbok
		...
		If (error="")
                    ...
                    if (trim(aux)<>"") 
                        ...
                        gErrorDB="OK"
                    ELSE
                        gErrorDB="Error DB Fecha"
                        ErrorMessage "PMS5: DiaDeNegocio: Fecha vacia"
                    endif	
		Else 	
                    
                    ErrorMessage "PMS5: DiaDeNegocio: Error al obtener sql"
                    ErrorMessage error
		Endif	
                call ODBCbaja
	Else
            gErrorDB="ODBC Invalido"

...
Sub ProcessMealConsumption()
	...
	If(gbliConsumeQty > 0)
		...
		If 	(gblsCustomerClass = CUST_CLASS_DISCOUNT)
			...
			if (gbliDiscObjNum>16 and gbliDiscObjNum<25) 
				LoadKybdMacro Key(KEY_TYPE_MENU_ITEM, 9106084)
			elseif (gbliDiscObjNum>320 and gbliDiscObjNum<325)
                                LoadKybdMacro Key(KEY_TYPE_MENU_ITEM, 900070)
                        endif
...
Sub AskForGuestId(Ref cardId_, Ref dni_, Ref payrollId_)
	...
	Window 2,50, "Identificacion"
		...
		//Display 1, 1, "Tarjeta: "
		Display 1, 1, "Dni    : "
                Display 2, 1, "Version 2.0 21/11/2013 "
...
Sub ValidateCustomer(Ref cardID_, Ref custID_, Ref payrollId_, Ref name_, Ref custClass_, Ref discObjNum_, Ref valid_)
	...
	If iDBOK
		...
		// Format query	
 		Format sqlCommand As 	"SELECT  CM.CardID, CM.CustomerID, CM.PayrollID, CM.Extrafield8 AS EmpName,       ", \
								"        CM.ExtraField1 AS CustClass, CM.ExtraField2 AS DiscountObjNum                                         ", \ 
								"FROM    custom.customer_meals CM                                                                              ", \
						  		"WHERE   (CM.CardID = '", cardID_,"' OR CM.CustomerID = '", custID_, "' OR CM.PayrollID = '", payrollId_, "')  "										

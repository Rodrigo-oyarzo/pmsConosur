...
Sub ProcessMealConsumption()
	...
	If(gbliConsumeQty > 0)
                Call CantidadDescuentos(gblsCustomerName,cantdesc)
                if (cantdesc>0)
                    Format mensaje AS "El empleado tiene ",cantdesc," breaks el dia de la fecha"
                     InfoMessage "Comida Break",mensaje
			ENDIF
		If 	(gblsCustomerClass = CUST_CLASS_DISCOUNT)
			...
			LoadKybdMacro Key(KEY_TYPE_MENU_ITEM, 9106084)
...
Sub AskForGuestId(Ref cardId_, Ref dni_, Ref payrollId_)
	...
	Window 2,50, "Identificacion"
		...
		//Display 1, 1, "Tarjeta: "
		Display 1, 1, "Legajo (2.1): "
		//Display 1, 1, "Legajo : "

		//DisplayInput 1, 10, cardId{16},"(max. 16 carac.)"
		DisplayInput 1, 14, dni{16},"(max. 16 carac.)"
...
Sub ValidateCustomer(Ref cardID_, Ref custID_, Ref payrollId_, Ref name_, Ref custClass_, Ref discObjNum_, Ref valid_)
	...
	If iDBOK
		...
		// Format query	
 		Format sqlCommand As 	"SELECT  null, CM.pc_appl_id, 999999, long_last_name+' '+long_first_name,       ", \
								"        'D', 18                                         ", \ 
								"FROM    micros.emp_def CM                                                                              ", \
						  		"WHERE   (CM.payroll_id = '", custID_, "')  "
...
//
//******************************************************************
Sub CantidadDescuentos(Ref name_,Ref cantdesc_)
	...
	If iDBOK
		// Format query	
                 ...
                 IF (len(name_)>19)
                        name_=mid(name_,1,19)
                  ENDIF

                  
 		Format sqlCommand As 	"select count(distinct A.trans_seq)  from micros.ref_dtl A,micros.trans_dtl B,micros.dsvc_dtl C,micros.dsvc_def D ,micros.rest_status E ", \
					" where A.trans_seq=B.trans_seq and B.business_date=E.business_date and B.trans_seq=C.trans_seq ", \ 
                                        " and C.dsvc_seq=D.dsvc_seq and D.obj_num=18  and ref='",name_,"'" 
		...
		If sSQLErr = ""
			...
			If sRecordData <> ""
				Split sRecordData, DB_REC_SEPARATOR, cantdesc_
				valid = TRUE
			Else
				valid = FALSE

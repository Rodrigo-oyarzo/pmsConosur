...
Sub ProcessMealConsumption()
	...
	If(gbliConsumeQty > 0)
		...
		If 	(gblsCustomerClass = CUST_CLASS_DISCOUNT)
                        LoadKybdMacro Key(KEY_TYPE_MENU_ITEM, 100055)
...
Sub AskForGuestId(Ref cardId_, Ref dni_, Ref payrollId_)
	...
	Window 2,50, "Identificacion (Version 1.0)"
	...
	If kKeyPressed = @KEY_ENTER
		...
		Format dni 		As Trim(dni)
                Format aux as mid(dni,1,len(dni)-1)
                Format dni_ as aux,"-",mid(dni,len(dni),1)
		...
		// Format query	
 		Format sqlCommand As 	"SELECT  null, CM.pc_appl_id, 999999, long_last_name+' '+long_first_name,       ", \
								...
								"FROM    micros.emp_def CM ,micros.emp_class_def B                                                                             ", \
						  		"WHERE   (CM.payroll_id = '", custID_, "') and Cm.emp_class_seq=B.emp_class_seq "
